@tool
@icon("res://addons/scene_map/scene_palette.svg")
...
func _get(property: StringName):
	if property.begins_with("items/"):
		...
		if !item_map.has(item_id):
			#print(property)
			push_error("_get - Item ID is not present in palette.");
			return null;
...
func remove_item(item_id: int) -> void:
	#print("remove_item called.")
	if item_id < 0:
		push_error("Item ID cannot be negative.");
	if !item_map.has(item_id):
		push_error("remove_item - Item ID is not present in palette.");
...
func get_item_name(item_id: int) -> String:
	if item_id < 0:
		...
		return "";
	if !item_map.has(item_id):
		push_error("get_item_name - Item ID is not present in palette.");
		return "";
...
func set_item_name(item_id: int, name: String) -> void:
	if item_id < 0:
		push_error("Item ID cannot be negative.");
	if !item_map.has(item_id):
		push_error("set_item_name - Item ID is not present in palette.");
...
func get_item_scene(item_id: int) -> PackedScene:
	if item_id < 0:
		...
		return null;
	if !item_map.has(item_id):
		push_error("get_item_scene - Item ID is not present in palette.");
		return null;
...
func set_item_scene(item_id: int, scene: PackedScene) -> void:
	if item_id < 0:
		push_error("Item ID cannot be negative.");
	if !item_map.has(item_id):
		push_error("set_item_scene - Item ID is not present in palette.");
