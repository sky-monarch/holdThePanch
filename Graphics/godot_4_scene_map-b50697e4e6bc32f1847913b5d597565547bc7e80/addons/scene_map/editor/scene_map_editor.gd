...
func handle_spatial_input(camera: Camera3D, event: InputEvent) -> bool:
	...
	if click_event:
		...
		elif click_event.button_index == MOUSE_BUTTON_WHEEL_DOWN && click_event.shift:
			if click_event.pressed:
				floor_control.value -= click_event.factor;
			return EditorPlugin.AFTER_GUI_INPUT_STOP;
...
func _handle_input(camera: Camera3D, point: Vector2) -> int:
	if !cursor:
		return EditorPlugin.AFTER_GUI_INPUT_PASS;
...
func _update_palette(palette: ScenePalette) -> void:
	#print("scene_man_editor, _update_palette called.")
	...
	for item_id in palette.get_item_ids():
		...
		if !search_text.is_empty() && !search_text.is_subsequence_ofn(name):
			continue;
		...
		palette_list.set_item_icon(item_count, get_theme_icon("PackedScene", "EditorIcons"));
